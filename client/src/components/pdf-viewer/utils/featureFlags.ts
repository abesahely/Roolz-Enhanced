...
interface FeatureFlags {
  // Whether to use the new PDF viewer implementation
  useNewPDFViewer: boolean;
  
  // Whether to use the new annotation tools
  useNewAnnotationTools: boolean;
  
  // Advanced features that might be separately toggled
...
export interface PDFViewerSelectorProps {
  // Whether to use the new implementation regardless of flags
  forceNew?: boolean;
  
  // Render props for legacy and new implementations
 ...
 * @returns JSX.Element from either the legacy or new renderer
 */
export const PDFViewerSelector = (props: PDFViewerSelectorProps): JSX.Element => {
  const { forceNew, legacyRenderer, newRenderer } = props;
  
  // Check feature flag or force parameter
  const useNew = forceNew || isFeatureEnabled('useNewPDFViewer');
  
  // Return the appropriate renderer
  return useNew ? newRenderer() : legacyRenderer();
};